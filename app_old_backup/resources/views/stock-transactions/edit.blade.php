                    ...
                    <form action="{{ route('stock-transactions.update', $stockTransaction) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('transaction_type') is-invalid @enderror" id="transaction_type" name="transaction_type" required>
                                ...
                                <option value="in" {{ old('transaction_type', $stockTransaction->transaction_type) == 'in' ? 'selected' : '' }}>Stock In</option>
                                <option value="out" {{ old('transaction_type', $stockTransaction->transaction_type) == 'out' ? 'selected' : '' }}>Stock Out</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('quantity') is-invalid @enderror" id="quantity" name="quantity" value="{{ old('quantity', $stockTransaction->quantity) }}" min="1" required>
                            @error('quantity')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('location_id') is-invalid @enderror" id="location_id" name="location_id" required>
                                ...
                                @foreach($locations as $location)
                                    <option value="{{ $location->id }}" {{ old('location_id', $stockTransaction->location_id) == $location->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3">{{ old('notes', $stockTransaction->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
